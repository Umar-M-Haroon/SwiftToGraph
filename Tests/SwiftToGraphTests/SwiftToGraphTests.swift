...
"""
        ...
        do {
            ...
            graph3.edges.forEach({print("edge:", graph3[$0.u], "->", graph3[$0.v])})
            XCTAssertEqual(graph3.nodes.count, 16)
//            parser.addFunctionCalls()
            parser.removeSystemFunctions(graph: graph3)
            print("-----------------------")
            XCTAssertEqual(graph3.nodes.count, 8)
            graph3.nodes.forEach({print("node:", $0.description)})
            graph3.edges.forEach({print("edge:", graph3[$0.u], "->", graph3[$0.v])})
